...
import {
  Plus, Save, Trash2, ArrowLeft, Loader2, BarChart3,
  ChevronDown, ChevronUp, Copy, Sparkles, AlertTriangle
} from 'lucide-react';
...
export default function EditarRomaneio() {
  ...
  // Carregar dados do romaneio para edição
  useEffect(() => {
    const carregarDadosRomaneio = async () => {
      if (!id) return;
      setCarregando(true);
      try {
        ...
        Swal.fire({
          icon: 'error',
          title: 'Erro',
          text: err.message || 'Falha ao carregar dados do romaneio para edição.',
  ...
  const salvarEdicao = useCallback(async () => {
    ...
    try {
      ...
      const res = await window.electronAPI.updateRomaneio(payload);
      if (!res.success) {
        throw new Error(res.error || 'Erro ao atualizar romaneio');
      }
      ...
      await Swal.fire({
        ...
        text: 'As alterações foram salvas com sucesso no banco de dados.',
        confirmButtonColor: '#059669',
      ...
      Swal.fire({
        ...
        text: err.message || 'Houve uma falha inesperada ao tentar atualizar o romaneio.',
        confirmButtonColor: '#059669'
      });
    } finally {
      setSalvando(false);
    }
  }, [cliente, data, id, pacotes, tipoRomaneio, totalGeralM3, totalGeralML, resetForm, navigate]);
  ...
  if (carregando) {
    return (
      <div className="flex flex-col items-center justify-center min-h-[60vh] gap-4">
        ...
        <p className="text-slate-500 dark:text-slate-400 font-bold">Carregando dados do romaneio...</p>
      </div>
    );
  }
        ...
        <div className="glass-card p-6 md:p-8 flex flex-col md:flex-row md:items-center justify-between gap-6 relative overflow-hidden">
          <div className="space-y-2 relative">
            <div className="flex items-center gap-3">
              ...
              >
                <ArrowLeft size={14} /> Voltar ao Romaneio #{id}
              </Link>
              ...
              >
                <Sparkles size={13} /> {getTipoLabel()} • Alterar
              </button>
            </div>
            ...
            <p className="text-sm text-slate-400 font-medium">
              Atualize as informações de cubagem e dados cadastrais deste romaneio.
            </p>
          </div>
